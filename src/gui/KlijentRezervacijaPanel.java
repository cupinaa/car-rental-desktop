...
public class KlijentRezervacijaPanel extends JPanel {
	...
	private void izvrsiPretragu() {
		...
		try {
			...
			for (ModelVozila m : mp.getModeli()) {
				...
				// Najvažnije: da li postoji SLOBODAN primerak ovog modela za tražene datume?
				Vozilo slobodnoVozilo = rp.pronadjiSlobodnoVoziloZaModel(m, vp, pocetak, kraj);
				
				if (slobodnoVozilo != null) {
					// Prikazujemo model, jer Klijent rezerviše Model, a mi smo se uverili da ima bar 1 vozilo slobodno.
	...
	private void potvrdiRezervaciju() {
		...
		try {
			...
			// Moramo naći konkretno vozilo ponovo
			...
			if (odziv == JOptionPane.YES_OPTION) {
				...
				if (novaRez != null) {
					...
					tableModel.setRowCount(0); // Čistimo tabelu
				} else {
