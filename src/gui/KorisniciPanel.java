...
public class KorisniciPanel extends JPanel {
	...
	public KorisniciPanel(KorisniciPodaci kp, Korisnik ulogovaniKorisnik) {
		...
		setLayout(new BorderLayout()); // Zauzima celo dostupno mesto unutar glavnog prozora
		...
		btnDodaj.addActionListener(e -> {
			KorisnikForma kf = new KorisnikForma(kp, null, () -> osveziTabelu(), ulogovaniKorisnik); // null znači NOVI korisnik
			kf.setVisible(true); 
		});
		...
		btnObrisi.addActionListener(e -> {
			...
			// Kolona 0 u tabeli je ID korisnika
			...
			if (zaBrisanje != null) {
				...
				if (potvrda == javax.swing.JOptionPane.YES_OPTION) {
					kp.obrisiKorisnika(zaBrisanje); // Ovo briše iz fajla
					osveziTabelu(); // Ovo briše sa ekrana
				}
			}
		});
		...
		btnIzmeni.addActionListener(e -> {
			...
			if (zaIzmenu != null) {
				// Prosleđujemo selektovanog korisnika formi
